...
class HorizontalScrollerView: UIView {
	...
	// 1 make it easy to modify the layout at design time
	...
	// if a content view was hit, delegate is informed
	// and view is scrolled to center
	...
	// access an album cover from the scroller
	func view(at index :Int) -> UIView {
		return contentViews[index]
	}
	
	// reloads the scroller
	func reload() {
	...
	// Makes sure album being viewed is always centered
	private func centerCurrentView() {
		...
		// once view centered, inform delegate selected view has changed
		delegate?.horizontalScrollerView(self, didSelectViewAt: selectedIndex)
	}
}

// Detects user finished dragging inside scroll view
